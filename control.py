...
def controller(q1, q2, w1, w2, q1_desired, q2_desired):
    ...
    t1 = (3+2*math.cos(q2))*a1+(1+math.cos(q2)*a2+0.5*(2*math.sin(q2)*w1*w2+math.sin(q2)*w2*w2)+0.5*10*(2*math.sin(q1))+math.sin(q1+q2))
    t2 = (1+math.cos(q2))*a1+a2-0.5*math.sin(q2)*w1*w1+0.5*10*math.sin(q1+q2)
    return t1, t2